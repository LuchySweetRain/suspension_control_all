...
class PPOAgent:
    ...
    def train_trajectory(self, trajectory: dict[str, list], bc_anchor_weight: float = 0.0, bc_anchor_batch_size: int | None = None) -> tuple[float, float]:
        ...
        unsafe = torch.as_tensor(
            np.asarray(trajectory.get("unsafe", np.zeros(len(trajectory["obs"])))),
            dtype=torch.float32,
            device=self.device,
        ).unsqueeze(1)
        action_delta = torch.as_tensor(
            np.asarray(trajectory.get("action_delta", np.zeros(len(trajectory["obs"])))),
            dtype=torch.float32,
            device=self.device,
        ).unsqueeze(1)
