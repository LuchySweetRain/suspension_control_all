...
On a MuJoCo full-car benchmark with correlated ISO-style roads, pothole-roll, sine-roll, and mixed-road scenarios, STG-PPO improves over PPO from scratch on all core safety and feasibility metrics across three seeds.
In the full matrix, STG-PPO reduces unsafe steps from 21.0 to 0.2, action-delta RMS from 169.16 N to 11.05 N, actuator tracking RMS from 252.41 N to 16.49 N, and policy projection error from 0.4804 to 0.0005.
Against SAC, STG-PPO is slightly lower in return but substantially smoother and less projection-dependent.
...
\begin{itemize}
    ...
    \item We report ablations showing that STG-PPO consistently repairs unsafe PPO exploration across three seeds and is competitive with SAC while using far smoother and more actuator-feasible commands.
\end{itemize}
...
Table~\ref{tab:full} summarizes learned-controller means.
Compared with PPO from scratch, STG-PPO reduces unsafe steps from 21.0 to 0.2 and projection error from 0.4804 to 0.0005.
The mean gate is 0.2688, indicating that the policy remains near passive on easy segments and opens mainly under stronger excitation.
...
SAC has slightly better mean return and zero unsafe steps in this short e20 run.
However, SAC uses much rougher and less actuator-feasible commands: action-delta RMS is 151.94 N versus 11.05 N for STG-PPO, tracking RMS is 226.72 N versus 16.49 N, and projection error is 0.2888 versus 0.0005.
TD3 is unstable in this fair-baseline setting, with 301 unsafe steps.
We therefore interpret the off-policy comparison as evidence of a tradeoff: STG-PPO is the smoother, more executable online-adaptation policy, while SAC remains a strong return/safety baseline.
...
First, the evidence is simulation-only and uses short e20 runs.
Second, SAC remains competitive in return and safety, so the current claim is actuator feasibility and online PPO repair, not universal superiority.
Third, the passive teacher is strong on several roads; harder road classes and payload/parameter variation are needed to show when active force is essential.
...
Across three seeds, STG-PPO consistently improves over PPO from scratch on return, safety, comfort, action smoothness, actuator tracking, and projection error.
In the full matrix, it is competitive with SAC while producing much smoother and more executable commands.
