...
\caption{Full-car active-suspension force model used by the benchmark. The diagram highlights sprung-body heave/pitch/roll dynamics, four corner unsprung masses, suspension and tire force paths, active actuator forces, and force/rate constraints.}
\label{fig:suspension_force_model}
\end{figure*}
...
\caption{STG-PPO control pipeline. Offline safe-teacher trajectories initialize PPO by behavior cloning. During online adaptation, the learned policy outputs bounded force increments, a safe-teacher improvement gate decides when active force is justified, and the actuator/safety projection layer produces the executed force used by the full-car plant and reward logger.}
...
\caption{Policy and training network architecture. Vehicle state, road preview, previous executed force, and safety margins are encoded into a shared latent representation feeding actor, critic, safe-teacher gate, and projection-aware auxiliary heads. The online objective combines PPO, behavior-cloning initialization, projection consistency, action smoothness, and safety penalties.}
