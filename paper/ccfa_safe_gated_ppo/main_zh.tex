...
在物理标定的 MuJoCo 全车基准上，包括相关 ISO 风格随机路、坑洞侧倾、正弦侧倾和混合路面，STG-PPO 相对从零训练 PPO 改善所有核心安全和可行性指标。
在完整 200 episode 矩阵实验中，STG-PPO 将不安全步数从 106.0 降至 0.0，将动作变化 RMS 从 142.92 N 降至 13.98 N，将执行器跟踪 RMS 从 225.09 N 降至 20.86 N，并将策略投影误差从 0.3626 降至 0.0001。
与 SAC 相比，STG-PPO 获得更高回报、更低车身/俯仰加速度和更低投影误差；但 SAC 在侧倾和执行器活动上更接近被动策略。
...
\begin{itemize}
    ...
    \item 通过消融实验表明，STG-PPO 能修复从零训练 PPO 的不安全探索问题，并在物理标定全车动力学下相对 TD3/SAC 保持竞争力和执行器可行性。
\end{itemize}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/suspension_force_model.png}
\caption{全车主动悬架受力模型示意图。图中标出车身垂向/俯仰/侧倾动力学、四角非簧载质量、悬架与轮胎受力路径、主动执行器作用力以及力幅值/力变化率约束。}
\label{fig:suspension_force_model_zh}
\end{figure*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/control_pipeline.png}
\caption{STG-PPO 控制流程。离线安全教师轨迹通过行为克隆初始化 PPO；在线适应阶段，策略输出有界作用力增量，安全教师改进门控决定何时允许主动作用力，执行器/安全投影层产生实际执行力，并输入全车模型和奖励日志。}
\label{fig:control_pipeline_zh}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/network_architecture.png}
\caption{策略与训练网络结构。车辆状态、道路预瞄、上一时刻执行力和安全裕度被编码为共享隐变量，并分别输入 actor、critic、安全教师门控和投影感知辅助头。在线目标结合 PPO、行为克隆初始化、投影一致性、动作平滑性和安全惩罚。}
\label{fig:network_architecture_zh}
\end{figure*}
...
物理标定基准使用 $1500\,\mathrm{kg}$ 簧载车身、$2160\,\mathrm{kg\,m^2}$ 俯仰转动惯量、$460\,\mathrm{kg\,m^2}$ 侧倾转动惯量、每轮 $59\,\mathrm{kg}$ 非簧载质量、前/后悬架刚度 $35000/38000\,\mathrm{N/m}$ 以及轮胎刚度 $190000\,\mathrm{N/m}$。
...
表~\ref{tab:full_zh} 总结物理标定 200 episode 矩阵的学习控制器均值。
相较从零训练 PPO，STG-PPO 将不安全步数从 106.0 降至 0.0，将投影误差从 0.3626 降至 0.0001。
平均门控为 0.1878，说明策略在简单路段保持接近被动，仅在更强激励下打开主动控制。
...
在物理标定 e200 实验中，STG-PPO 的平均回报高于 TD3 和 SAC，并具有更低的车身/俯仰加速度和投影误差。
然而，SAC 更接近被动低动作策略：其侧倾加速度更低，执行器活动几乎为零，而 STG-PPO 只在少量状态上打开门控。
因此，离策略对比应被解读为一种权衡：STG-PPO 是更强的投影感知在线适应策略，而 SAC 和被动控制仍是当前路面集合上的强低动作参考。
...
第一，当前证据仅来自仿真，并且仍只覆盖有限的路面分布和随机种子。
第二，在物理标定路面集合上，被动控制仍是最强名义参考，因此本文当前主张是执行器可行性和在线 PPO 修复，而非主动控制相对被动悬架的全面优越性。
第三，SAC 作为低动作基线仍然具有竞争力；需要更困难的路面、载荷变化和参数随机化来证明主动作用力何时不可或缺。
第四，当前门控使用手工阈值。
...
在物理标定完整时长矩阵中，STG-PPO 相对从零训练 PPO 在回报、安全、舒适性、动作平滑性、执行器跟踪和投影误差上均稳定改善。
它也在回报和投影一致性上优于 TD3/SAC，但被动控制和 SAC 仍是强低动作参考。
